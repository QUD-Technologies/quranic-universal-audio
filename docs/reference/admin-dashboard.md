...
Routes: `routes/admin/intake_plan.py`; service `services/admin/intake_plan/`; wire `qua_shared/schemas/wire/intake_plan.py`. Credentials match the align routes: session cookie (POST/PUT same-origin) or an owner bearer. Full flow (enumerate → match → identity → mint → align): [align-pipeline.md § Online intake](align-pipeline.md#online-intake-plan--mint--align).

**Intake plan panel.** An intake row in the Requests tab renders `tabs/dashboard/components/admin/intake/IntakePlanPanel.svelte`: build/rebuild the plan, a summary (host, coverage, errors/warnings, `IntakePlanSummary`), the entry → chapter table with confidence badges (`IntakePlanEntries`), the identity form (slug / reciter / channel / source, new source name for a YouTube uploader; `IntakeIdentityForm`), and the GPU/CPU Align CTA (`IntakeAlignCta`), blocked while there are unsaved edits or blocking errors, or when the chosen lane's budget is spent.
