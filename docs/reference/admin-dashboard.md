...
| `ReleasesTsSettings.svelte` | Unified generate/regenerate form — beam 50 (single pass), collapsible Advanced, inline Full-vs-Affected scope chooser. Launches via `generateTimestamps`. |
...
The Submit-recitation wizard's two **slugless** types — `existing_reciter_new_combo` and `new_reciter` — carry an audio **source** (direct links / playlist; a dropped CSV/JSON file is normalised into `links[]` client-side). They land in the unified `requests` table with `slug = NULL`, everything parked in `payload` (`reciter_id`, `proposed_edits` (ProposedEdits-shaped), `source`, three required `attestations` (distribution/links-verified/storage rights — recorded for audit), cached `probe`, and the owner's intake `plan`). Service: `services/admin/intake.py`; validation `intake_validation.py`; probe `intake_probe.py`; schemas `qua_shared/schemas/wire/intake_requests.py` (codegen'd).
...
| `GET /admin/intake/<id>/plan` | `intake.ingest` (owner by default). The stored `payload.plan` + live check (`IntakePlanView`: coverage, errors, warnings, channel/source options), or `{plan: null}`. |
| `POST /admin/intake/<id>/plan` | `intake.ingest`. (Re)enumerate the source in a background thread (`status: enumerating → ready \| failed`); the panel polls `GET`. |
| `PUT /admin/intake/<id>/plan` | `intake.ingest`. Save the owner's review (`IntakePlanUpdate`: per-entry chapters, edited entries become `manual`, plus the identity). |
| `POST /admin/intake/<id>/align` | `intake.ingest` + `intake.align` (shared GPU/CPU budget applies; `IntakeAlignRequest.device`). Mints the delivery (`intake.ingest()`) and starts its align run; returns `IntakeAlignResponse` (the mint lands even if the run start is refused). |

Routes: `routes/admin/intake_plan.py`; service `services/admin/intake_plan/`; wire `qua_shared/schemas/wire/intake_plan.py`. Credentials match the align routes: session cookie (POST/PUT same-origin) or an owner bearer. Full flow (enumerate → identity → mint → align, surahs detected by the aligner): [align-pipeline.md § Online intake](align-pipeline.md#online-intake-plan--mint--align).

**Intake plan panel.** An intake row in the Requests tab renders `tabs/dashboard/components/admin/intake/IntakePlanPanel.svelte`: build/rebuild the plan, a summary (host, coverage, errors/warnings, `IntakePlanSummary`), the file table with a per-file Include toggle — titles are never mapped to chapters, the align run detects them; `links` show their contributor chapters read-only (`IntakePlanEntries`), the identity form (slug / reciter / channel / source, new source name for a YouTube uploader; `IntakeIdentityForm`), and the GPU/CPU Align CTA (`IntakeAlignCta`), blocked while there are unsaved edits or blocking errors, or when the chosen lane's budget is spent.

**There is no accept step.** A submission lands `pending` and is directly alignable. The owner's decision to *align* it (plan → Align on the row) IS the acceptance, so the Requests **Open** facet shows probe / send-back / discard plus the plan panel (no Accept button). For a **new reciter** the canonical `reciter_id` is proposed by the plan's identity and confirmed by the owner, not stamped up front. Minting flips the row to `accepted` + back-fills the slug (so the **Accepted** facet means *minted*); the new slug's align run then shows on its slug row. `request.intake_*` are audit-only events (slugless, not in `_HANDLERS`) — silently dropped from both activity rails, no classification entry needed.
