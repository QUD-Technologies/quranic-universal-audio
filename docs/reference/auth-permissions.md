...
`is_oauth_configured()` = a client id resolved by `oauth_client_credentials()` AND `get_session_secret()` doesn't raise `MissingSecret`. That pair comes from the `OAUTH_CLIENT_ID`/`OAUTH_CLIENT_SECRET` **Space secrets**, holding an OAuth app we registered — the Space README omits `hf_oauth`, both because HF's auto-provisioned app refuses a custom domain's `redirect_uri` and because leaving it on makes HF inject these same names and collide with the secrets (`CONFIG_ERROR`). Scopes come from `OAUTH_SCOPES`, default `openid profile`; the callback reads only `sub` and `preferred_username`/`name`, so no `email` scope is needed.
...
| Piece | Truth |
|---|---|
| Registry (data) | `qua_shared/schemas/config/capabilities.py` — `Capability{id, group, label, description, anon_eligible, owner_only_fixed, default_grants}` + `CAPABILITIES` tuple. `default_grants` encodes **today's exact behavior** (empty override table == legacy authz, guarded by `tests/services/test_capabilities.py::test_baseline_parity`). Backend-only; NOT codegen'd to the FE. |
