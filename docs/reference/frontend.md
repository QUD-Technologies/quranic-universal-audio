...
**Sample word review.** Sample rows may carry historical `word_timings` from `detailed.json`. Both segment read routes expose the intervals, and `SegmentRow` highlights the sounding word during playback only in sample mode. `utils/samples/word-timing.ts` owns interval lookup and rejects timings whose first/last locations no longer match the row reference; save handling drops timings after a trim or reference change and preserves them across unrelated chapter rebuilds.
