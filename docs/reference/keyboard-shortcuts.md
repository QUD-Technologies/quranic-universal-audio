...
- **Pieces are never bounded individually.** A card's pieces are contiguous slices of one window, so cutting the range at a piece edge stopped the audio early and the advance then started a FRESH play of the next piece (heard as "piece 1 stops early and jumps to piece 2"). The card publishes `groupEndMs` (its last piece's end) to its member rows, `playFromSegment` extends the bounded range to it, and the audio runs through the whole group while only the cursor walks piece to piece. The nav cursor follows that walk — republished by the staged draw branch in `drawActivePlayhead` and by the pair crossing in `onSegTimeUpdate` — so ↑/↓ and the advance still step from the piece actually playing, and the advance fires once, at the group's end.
