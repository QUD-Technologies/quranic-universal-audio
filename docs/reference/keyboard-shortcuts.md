...
- **`setWasl` (1 / 2)**: each `WaslBoundary` picker publishes its commit to its `GenericIssueCard` via `onCommitReady(uid, commit)`, keyed by the uid of the piece above it. `utils/validation/wasl-binding.ts::waslCommitForPiece` then gives every main member row the commit for the boundary **directly above** that row, and the **first** piece (which has none above) doubles for the boundary **below** it. So a focused piece always maps to exactly one boundary, and a two-piece cross verse resolves to its single boundary from either side — no ambiguity even with three or more pieces (piece 0 → boundary 0|1, piece 1 → boundary 0|1, piece 2 → boundary 1|2, …). A row with no boundary on either side publishes no `setWasl`, and the keys are then unhandled. `1`/`2` are ordinary rebindable `accordion`-pool actions; `←`/`→` keep seeking.
- **Labelling is a pure data edit**: the keyboard flavour (`commit(value, { silent: true })` — the one handed up via `onCommitReady`) skips the post-split chain hand-off, which would open a ref-edit on the next piece and move focus; it leaves `accordionNavCursor`, the port and the live range untouched. So `1`/`2` never steal focus and never change where ↑/↓ goes next; the audio keeps playing exactly as it was, and when the piece it is bounded to ends, autoplay advances to the next segment like any other — an earlier attempt suppressed that advance and turned it into a spurious pause after the first piece. Clicking the picker keeps the chain behaviour — that is the designed post-split flow.
...
- Each stop carries its own `startMs`/`endMs` (from `data-seg-start`/`data-seg-end`), and `playFromSegment`'s `piece` option seeks to `startMs` so ↑/↓ lands on the piece itself.
- **Pieces are never bounded individually.** A card's pieces are contiguous slices of one window, so cutting the range at a piece edge stopped the audio early and the advance then started a FRESH play of the next piece (heard as "piece 1 stops early and jumps to piece 2"). The card publishes `groupEndMs` (its last piece's end) to its member rows, `playFromSegment` extends the bounded range to it, and the audio runs through the whole group while only the cursor walks piece to piece. The nav cursor follows that walk, and so do the pair and `segCurrentIdx`: `followGroupPlayhead()` runs on the bounded-range tick and moves all three onto whichever piece contains the playhead. It has to live there — `onSegTimeUpdate` bails whenever the port isn't playing the *active* chapter (accordion cards routinely mount rows from another chapter) and otherwise scans only the main list's filtered slice, which need not contain the card's rows; without it the highlight and cursor stuck on the piece that started the play while the audio ran on into the next. The advance then fires once, at the group's end.
- `ensureBoundedRange()` rebuilds `_segRange` from the pair's own segment, which after a split is ONE piece — so it honours `_activeGroupEndMs` (the group bound the current play was started with). Otherwise any mid-play rebuild (autoplay or chime toggle, edit-mode exit) re-bounds a group play to the first piece's end and the audio stops there.
...
- **`reanchorPlayingAfterSplit(chapter, pieces)`** (called from `split-commit.ts` after `reconcilePlayingAfterMutation`) moves the playing pair onto the piece **the user is on**. The UID walk lands on piece 0 (a split preserves the parent UID there), so labelling from the second piece jumped the highlight back to the first and froze the cursor at its edge. Target order: `accordionNavCursor` (a split preserves the staged children's UIDs, so it still matches) → the piece containing the playhead → the last piece the playhead has reached (a finished bounded play parks the playhead exactly ON a piece's end, which no window contains — that belongs to the piece that just played, not piece 0).
- It moves **`segCurrentIdx` in lockstep** with the pair. `updateSegHighlight` forces `playingSegmentIndex` back onto `segCurrentIdx` on every rAF frame, so a pair moved on its own is reverted within a frame and the highlight + cursor sit on the first piece — the same trap `_maybeSkipDeletedGap` documents for its own jump.
- Beyond the pair it touches **nothing**, and never the port or the live `AudioRange`: `dispose()` pauses and `start()` re-seeks + plays, so re-pointing a range mid-play is heard as a pause-and-jump-back. There is nothing to re-point anyway — a split leaves each piece's window exactly where its staged slice was, so the existing range stays correct. (An earlier attempt did dispose+start and was heard as exactly that; the range is now left alone.)
