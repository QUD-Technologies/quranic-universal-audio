...
Only main-list sample rows with a word list matching their reference show **Words** beneath the play/flag controls. Opening it replaces the row actions with a dedicated timing view and loops the segment. Click the waveform or a word to seek. Each interior line is one shared boundary: dragging it changes the previous word's end and next word's start together, so no gap opens. The first start and last end remain independent; dragging a whole word carries the neighboring shared edges while preserving positive word lengths. The lock on each word switches playback to a loop of that word; only one lock can be active, and the loop follows boundary edits immediately. Unlocking returns to the segment loop. **Save word timings** sends a single `edit_word_timings` operation through the regular authenticated `/api/seg/save/<sample--slug>/<chapter>` path. The server checks the original boundaries to reject stale edits, validates ordered word bounds inside the segment, writes `detailed.json`, records an inverse patch in `edit_history.jsonl`, and invalidates the segment cache. Pending segment edits must be saved first. The word editor does not change segment boundaries or references. Sample word timings use the benchmark's karaoke display policy: first word starts at the segment start, interior words meet, and the last word ends at the segment end, except for the two mujawwad recordings whose trailing segmenter silence remains unpadded.
...
### Staged cross-verse split (label-to-save)

A cross-verse card whose seg has a usable sidecar entry (`kind: cross_verse`, `refs.length == cursors.length + 1`, cursors strictly inside the seg — `utils/validation/staged-split.ts::resolveStagedSplit`) does **not** show the single row + Auto Split button. `GenericIssueCard.svelte` renders the N **staged pieces** up front (`buildStagedChildren`: parent sliced at the cursors, refs from the sidecar, piece 0 keeps the parent uid/index, later pieces take uids memoised per parent in `stores/staged-split.ts`) with a `WaslBoundary` picker between each pair. Staged rows (`SegmentRow staged`) look and act like any other row — play seeks to the piece's own start and runs to the parent's end so the cursor walks card to card (`stagedPlayheadWindow` names the piece holding the cursor, and only that row shows as playing), the waveform is drawn from the piece's own range through the `data-hist-*` attributes — except that they register in the playback row registry carrying themselves as the entry's `segOverride` (they share the parent's `(chapter, index)`, so without it `drawActivePlayhead` would resolve the parent's undivided window from the store and the piece would show no cursor — it paints each piece against its own window, only while the playhead is inside it, and erases a piece once on exit; `_pickHandoffMountId` skips these entries so a programmatic edit never claims a staged mount) and every edit action (Adjust / Split / Merge / Delete / Edit Ref / flag / ref click) first calls `onStagedActivate` → the card's `materializeStaged()` (commit with the picks so far; unanswered boundaries commit as WAQF and are marked `pendingWaslConfirm` so their pickers keep asking), waits a tick, then runs against the now-real piece (same uid, same mounted row). On a missed-waqf card the commit cuts only the boundaries already answered WAQF (never an unanswered one); when none is, or the piece was merged away, `onStagedActivate` answers false and the action is a no-op.

Labelling is the whole edit. Picks are session state (`stagedWaslPicks`, keyed by parent uid); **nothing is dispatched until every boundary is labelled** (or an edit action materialises the split early). The last pick calls `utils/edit/split-commit.ts::commitSplit` with `refs`, `wasls[]`, the memoised `newUids` and `contextCategory = <card category>`, then `finalizeSplit` — one `split_segment` op on the normal op-log + autosave path, no ref-edit chain. Because the op's `command.wasls` is set, `execute.ts::isChapterHalfDirty` treats it as complete (no ref-confirm chain to wait for) so autosave fires. Once committed the real pieces take over via the split-group members and the pickers fall back to their default path (pending-split amend via `amendSegInOp`, or `set_is_wasl` after save). `confirmSplit` (canvas edit mode) shares the same `commitSplit` core and keeps its chain epilogue.

The same staged shape appears when a cross-verse seg surfaces in **another** per-segment category (low_confidence, repetitions, …); after commit those cards show the N pieces with a picker on every join that crosses a verse (`boundary-state.ts::isVerseBoundary`). **Ignore** on a generic card applies to every real member of the group (`realMembers`); while still staged only the parent is real and its `ignored_categories` are inherited by the pieces at commit.

**Boundary chips + filter.** The cross-verse accordion header shows **Unset · Wasl · Waqf** chips, each with a **boundary** count (a three-verse item contributes two). `utils/validation/boundary-state.ts::boundaryStates` reads, in order: committed/in-progress members (`is_wasl` per left member; a uid in `pendingWaslConfirm` or in the re-check set `waslRecheck` is unset) → staged picks → a single unset boundary for an unsplit seg. The chips are a multi-select filter (`stores/boundary-filter.ts`, default Unset only, persisted globally); an item passes when any of its boundaries is selected. The badge counts items that still hold an unset boundary. So that labelled items stay reviewable, the validate response keeps **resolved** cross-verse items: `services/validation/cross_verse_resolved.py` walks effective edit-history batches for `split_segment` ops launched from the card (`op_context_category == cross_verse`) or whose parent ref spanned verses, and emits `SegValCrossVerseItem{resolved: true}` for every root still live and single-verse. Resolved items are excluded from `category_counts`, and the FE mark-ready gate (`utils/validation/blocking-count.ts`) drops them too — only an unlabelled cross-verse blocks. Registry: `IssueDefinition.boundaryFilter` (FE-only). A re-asked boundary (`wasl_recheck` in the validate payload, see `validation.md`) renders its picker pending with neither choice highlighted until the reviewer answers (click or 1 / 2); the answer is always recorded as a `set_is_wasl` op, even when it matches the stored flag. The **Missed Waqf** accordion reuses the same staged pieces, pickers, chips and filter with inverted semantics (WAQF cuts, WASL drops the cursor; all WASL ignores) — `boundaryStates(item, ctx, 'missed_waqf')` counts a split item's cuts as waqf and its dropped cursors as wasl, and an ignored item's cursors as wasl. See `validation.md` (`missed_waqf` card).

**Timestamps side.** Generation runs on the timing Space; an unsplit compound ref is read as waṣl on the fly (`expand_cross_verse`, see [`dataset-and-releases.md`](dataset-and-releases.md)) and nothing is persisted — labelling in the editor is what records the reading. `set_is_wasl` is in `TS_AFFECTING_OP_TYPES` (both the Python source and the FE mirror in `utils/history/items.ts`): a relabel after a generation marks the timestamps stale.
