...
| File | Role |
|---|---|
| `__init__.py` | Orchestrator `validate_reciter_segments(reciter, include_boundary_review=True)` + `strip_boundary_review`. `ThreadPoolExecutor(8)` fan-out of the 8 independent bucket reads (`load_detailed` / `_load_resolved_idx_cached` / `load_probe_v2` / `load_hidden_pause` / `load_missed_waqf` / `load_false_split` / `load_unmarked_wasl` / `load_seg_verses`). `canonical=None` throughout (phonemizer out of runtime). Builds `category_counts` + per-category arrays + `split_group_index`. Re-exports registry symbols + classifier API. |
...
| route | handler | notes |
|---|---|---|
| `GET /api/seg/validate/<reciter>` | `seg_validate` | full-reciter validate. Cached via `cache.get/set_seg_validate_cache`; invalidated on save (`invalidate_seg_caches`). `orjson_cached_response`. **No per-chapter param** (dropped — accordions are global). The cached payload is complete; `hidden_pause` / `missed_waqf` / `false_split` / `unmarked_wasl` are redacted per viewer without `segments.view_boundary_review`. |
