...
**`missed_waqf` card.** It is the cross-verse accordion with its cuts from the item's own `boundary` (`stagedSplitFor('missed_waqf', …)` → `resolveStagedSplit` with `MISSED_WAQF_KINDS`; the Auto Split map is not consulted, so an `auto_split_v1` cross-verse entry for the same uid cannot hijack it). The card shows the staged pieces with the `WaslBoundary` WASL · WAQF picker (1 / 2) on every cursor; answering the last one commits ONE split at every cursor carrying the picks as `is_wasl` (`stagedCommit`, `contextCategory: 'missed_waqf'`), exactly as cross-verse does, and later picks amend `is_wasl` on the pieces. WAQF = the reciter stopped there; WASL = the cut is kept as a segment boundary read through. Session picks are keyed `missed_waqf:<uid>` (`stagedPickKey`) so a seg in both staged accordions keeps separate answers. `missed_waqf` is in `RESOLVES_BY_EDIT_CATEGORIES`, so the split resolves the root (piece 0 keeps the sidecar uid), and `_build_detail_lists` emits every live sidecar seg whose flag is suppressed as `resolved: true`, so labelled items stay listed and relabelable. The header chips Unset · Wasl · Waqf (`boundaryFilter`, default Unset) count boundaries via `boundary-state.ts` with `category = 'missed_waqf'`: every join of a split item by its left piece's `is_wasl`, otherwise the staged picks (unanswered = unset). The evidence strip shows the score only.
