...
| 11 | `missed_waqf` | info | per_segment | `segment_uid ∈ missed_waqf_v1.json::by_uid` (an offline phoneme + silence detector heard the reciter stop inside the segment; `load_missed_waqf`); ignored or split-from-card items stay listed as `resolved: true` (excluded from counts) | server | generic | `classifier.py` + `_build_detail_lists` |
| 12 | `qalqala` | info | per_segment | persisted `qalqala_letter` non-null (fall-through `compute_qalqala_letter`) | server | generic | `classifier.py` + `segments/qalqala.py` | **owner-only** |
| 13 | `muqattaat` | info | per_segment | `s_word==1 and (surah,s_ayah) ∈ MUQATTAAT_VERSES` | server | generic | `classifier.py` |
| 14 | `basmala_amin` | info | per_segment | per-chapter scan: first seg overlapping `1:1`, last overlapping `1:7`, + missed-Basmala augmentation gated on `len(deleted_basmala_chapters) >= MISSED_BASMALA_FLAG_MIN_DELETED` | server | generic | `detail.py::_build_detail_lists` |
| 15 | `hidden_pause` | info | per_segment | `segment_uid ∈ hidden_pause_v1.json::by_uid` (offline re-segmentation heard a pause inside the segment; `load_hidden_pause`) | server | generic | `classifier.py` + `_build_detail_lists` |
| 16 | `false_split` | info | per_segment | `segment_uid ∈ false_split_v1.json::by_uid` (offline re-segmentation heard continuous speech across the segment's end; `load_false_split`) | server | generic | `classifier.py` + `_build_detail_lists` |
| 17 | `unmarked_wasl` | info | per_segment | `segment_uid ∈ unmarked_wasl_v1.json::by_uid` (every re-segmentation arm read through the verse-to-verse join to the next segment and the delivery never marked `is_wasl`; `load_unmarked_wasl`) | server | generic | `classifier.py` + `_build_detail_lists` |

`hidden_pause` / `missed_waqf` / `false_split` / `unmarked_wasl` are **review-only**: their arrays + `*_meta` blocks are emitted only to viewers holding `segments.view_boundary_review` (maintainer default; owner superuser) — the route caches the full payload per reciter and `strip_boundary_review` redacts per request (counts zeroed, keys absent → the FE hides the accordions). None is in `BLOCKING_COUNT_KEYS` or `REQUIRED_GUIDE_KEYS`. Each item carries the sidecar payload under `boundary` (hidden_pause and missed_waqf: `cursors` / `refs` / `score` / `cuts[]` with per-axis `evidence`; false_split and unmarked_wasl share one shape: `next_uid` / `axes` / `gap_ms` / `score` / `word` / `final_class` / `is_wasl` / `evidence`); `score` = agreeing axes × 1000 + min(gap_ms, 999) and all four accordions default-sort by it descending. Auto Split on a `hidden_pause` row reads the cut from `/api/seg/auto-split` — `services/segments/auto_split.py` merges `missed_waqf_v1` and `hidden_pause_v1` entries that carry `refs` (kinds `missed_waqf` / `hidden_pause`; per uid `auto_split_v1` wins, then `missed_waqf`); entries with `refs: null` fall back to plain Split. A `false_split` row resolves with the row's Merge ↓ (auto-suppress on edit, like every per-segment category). An `unmarked_wasl` row opens with the next segment in context (`ACCORDION_CONTEXT` `next_only`) and the WASL/WAQF picker between the two; the fix is `set_is_wasl` on the left segment (or Merge ↓ if it is one utterance) — never a split, so the row is not an Auto Split candidate and `auto_split.py` does not read the sidecar.
...
**`missed_waqf` card.** It behaves like the cross-verse accordion. The card stages its own item's `boundary` (`stagedSplitFor('missed_waqf', …)` → `resolveStagedSplit` with `MISSED_WAQF_KINDS`; the Auto Split map is not consulted, so an `auto_split_v1` cross-verse entry for the same uid cannot hijack it) and shows the pieces with the `WaslBoundary` WASL · WAQF picker (1 / 2) on every cursor. The semantics are inverted from cross-verse: WAQF = the reciter stopped → cut there; WASL = no stop → drop that cursor and merge its pieces' refs. Once every cursor is answered, `stagedCommit` commits ONE split at the WAQF cursors only (`contextCategory: 'missed_waqf'`, no `is_wasl` set), or dispatches Ignore for `missed_waqf` when every cursor is WASL. An edit action on a staged piece before then commits only the cursors answered WAQF so far — an unanswered cursor is a suspect and never becomes a cut; with none answered WAQF the seg is left untouched and the action is a no-op (as it is when the acted piece was merged away). Committed pieces reuse the staged uid of the piece that starts where they start. Session picks are keyed `missed_waqf:<uid>` (`stagedPickKey`) so a seg in both staged accordions keeps separate answers. Labelled items stay listed and relabelable: `missed_waqf` is in `RESOLVES_BY_EDIT_CATEGORIES`, so the card's split resolves the root (piece 0 keeps the sidecar uid), and `_build_detail_lists` emits every live sidecar seg whose flag is suppressed (ignored or resolved by edit) as `resolved: true`. The header chips Unset · Wasl · Waqf (`boundaryFilter`, default Unset) count boundaries via `boundary-state.ts` with `category = 'missed_waqf'`: a split item counts each cut as waqf and each proposed cursor the split dropped as wasl; an item ignored for `missed_waqf` counts every cursor as wasl; otherwise the staged picks (unanswered = unset). Relabelling: on an ignored item a WAQF pick splits at that cursor; on a split item the picker between pieces shows WAQF and a WASL pick merges the two pieces (`mergeAdjacent`), and merging the last cut away ignores the item.

`owner_only` (registry field, default `False`) makes a category **invisible to everyone but an owner** — for a non-owner it may as well not exist. `ValidationPanel.svelte` drops the row from `buildBaseDescriptors` (gated on the `isOwner` store), so there is no accordion, no count badge and no filter entry; `REQUIRED_GUIDE_KEYS` is derived as `ALL_GUIDE_KEYS` minus owner-only categories so the guide never gates a first edit or shows in the guides checklist (the guide stays registered and readable from the owner's accordion `?`, and `GUIDE_VIEW_KEYS` in `inspector/constants.py` still accepts the view POST); and the category must never appear in `BLOCKING_COUNT_KEYS` — `mark-ready-copy.test.ts` asserts that. Currently: `boundary_adj` and `qalqala`. This is a display gate, not authz: the validate response still carries the array (unlike the `view_boundary_review` redaction above, which is capability-gated server-side).
...
| File | Role |
|---|---|
| `__init__.py` | Orchestrator `validate_reciter_segments(reciter, include_boundary_review=True)` + `strip_boundary_review`. `ThreadPoolExecutor(9)` fan-out of the 9 independent reads (`load_detailed` / `_load_resolved_idx_cached` / `load_probe_v2` / `load_hidden_pause` / `load_missed_waqf` / `load_false_split` / `load_unmarked_wasl` / `load_seg_verses` / `open_wasl_recheck_uids`). `canonical=None` throughout (phonemizer out of runtime). Builds `category_counts` + per-category arrays + `split_group_index` + `wasl_recheck`. Re-exports registry symbols + classifier API. |
| `wasl_recheck.py` | `open_wasl_recheck_uids(reciter)` — `wasl_recheck_v1.json` uids not yet answered by an edit-history op saved after `_meta.created_at` (see **Wasl re-check** above). |
...
| route | handler | notes |
|---|---|---|
| `GET /api/seg/validate/<reciter>` | `seg_validate` | full-reciter validate. Cached via `cache.get/set_seg_validate_cache`; invalidated on save (`invalidate_seg_caches`). `orjson_cached_response`. **No per-chapter param** (dropped — accordions are global). The cached payload is complete; `hidden_pause` / `missed_waqf` / `false_split` / `unmarked_wasl` are redacted per viewer without `segments.view_boundary_review`; `wasl_recheck` is not. |
